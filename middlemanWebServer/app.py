...
async def post_mod_action(action: ModActionIn):
    ...
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            "INSERT INTO mod_actions(time, action, player, reason, extra_json) VALUES(?,?,?,?,json(?))",
            [when, action.action, action.player, action.reason, action.extra and json.dumps(action.extra) or None],
        )
        await db.commit()
    return {"ok": True}
...
async def enqueue_command(command: str, payload: Dict[str, Any]):
    ...
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            "INSERT INTO commands(time, command, payload_json) VALUES(?,?,json(?))",
...
async def upsert_player_state(s: PlayerStateIn):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            "INSERT INTO player_state(player, strikes, banned, vestige) VALUES(?,?,?,?) ON CONFLICT(player) DO UPDATE SET strikes=excluded.strikes, banned=excluded.banned, vestige=excluded.vestige",
            [s.player, s.strikes, s.banned, s.vestige],
        )
        await db.commit()
    return {"ok": True}


@app.get("/player/state", dependencies=[Depends(require_admin)])
async def get_player_state(player: str):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute("SELECT * FROM player_state WHERE player = ?", [player])
        row = await cur.fetchone()
    return {"ok": True, "result": dict(row) if row else {"player": player, "strikes": 0, "banned": 0, "vestige": 0}}
